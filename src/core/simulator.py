...
def confronta_modelli_congestione(lunghezza: float = 10, tipo: str = "centrale"):
    """
    Confronta i tre modelli di congestione per capire le differenze.
    """
    print("=" * 60)
    print("Confronto modelli di congestione")
    print("=" * 60)
    print(f"Corridoio: {lunghezza}m, tipo='{tipo}'\n")

    affollamenti = [0.0, 0.3, 0.6, 0.9]
    modelli = ["lineare", "quadratico", "soglia"]

    print(f"{'Affollamento':<15}", end="")
    for mod in modelli:
        print(f"{mod:<15}", end="")
    print()
    print("-" * 60)

    for aff in affollamenti:
        print(f"{aff:<15.1f}", end="")
        for mod in modelli:
            sim = SimulatoreCosti(modello_congestione=mod, seed=42)
            tempo = sim.tempo_percorrenza(lunghezza, 9, aff, tipo)
            print(f"{tempo:<15.2f}", end="")
        print()


if __name__ == "__main__":
    # Demo rapida
    sim = SimulatoreCosti(seed=42)

    print("Test simulatore:")
    print(f"Corridoio 10m, ora 9:00, affollamento 0.7")
    print(f"Tempo: {sim.tempo_percorrenza(10, 9, 0.7, 'centrale'):.2f}s\n")

    confronta_modelli_congestione()